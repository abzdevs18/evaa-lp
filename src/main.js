...
function initCarousel() {
  ...
  function getConfig() {
    ...
    if (vw <= 400)
      return { cardWidth: 220, gap: 16, adjScale: 0.78, farScale: 0.7 };
    if (vw <= 640)
      return { cardWidth: 240, gap: 20, adjScale: 0.8, farScale: 0.72 };
    if (vw <= 960)
      return { cardWidth: 260, gap: 24, adjScale: 0.82, farScale: 0.74 };
  ...
  let activeIndex = cards.findIndex((c) =>
    c.hasAttribute("data-initial-active"),
  );
...
function initPageAnimations() {
  ...
  document
    .querySelectorAll(".card:not(.accent-top):not(.carousel-card)")
    .forEach((card, i) => {
      gsap.from(card, {
        scrollTrigger: {
          trigger: card,
...
function initHeroVideo() {
  ...
  if (video && video.paused) {
    video.play().catch(err => console.warn("Hero video autoplay blocked:", err));
  }
}
